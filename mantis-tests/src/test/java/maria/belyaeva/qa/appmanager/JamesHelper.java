...
public class JamesHelper {
    ...
    public List<MailMessage> waitForMail(String username, String passwd, long timeout) throws MessagingException {
        long now = System.currentTimeMillis();
        while (System.currentTimeMillis() < now + timeout) {
            List<MailMessage> allMail = getAllMail(username, passwd);
            if(allMail.size() > 0) {
                return  allMail;
            }
            try {
    ...
    private List<MailMessage> getAllMail(String username, String passwd) throws MessagingException {
        Folder inbox = openInbox(username, passwd);
    ...
    private Folder openInbox(String username, String passwd) throws MessagingException {
        store = mailSession.getStore("pop3");
        store.connect(mailserver, username, passwd);
    ...
    public void drainEmail(String username, String passwd) throws MessagingException {
        Folder inbox = openInbox(username, passwd);
